...
# Dataset options
parser.add_argument('--dataset_name', default='my', type=str)
...
def main(args):
    ...
    long_dtype, float_dtype = get_dtypes(args)  # GPU 사용하면 타입 변환

    logger.info("Initializing train dataset")
    train_dset, train_loader = data_loader(args, train_path)  # dataset, loader 반환
    ...
    while t < args.num_iterations:
        ...
        for batch in train_loader:
            ...
            # obs_traj: (8, ??, 2) 형태로 구성됨. 중간 부분은 매 데이터마다 다르기 때문에 알 수 없음.
            ...
            if t >= args.num_iterations:
                break
        time_end = time.time()
        train_time_total += time_end - time_start

    train_time_per_epoch = train_time_total / args.num_iterations
    print("Epoch 당 학습 시간: ", train_time_per_epoch, 's')
