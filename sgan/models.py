...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = nn.LSTM(
            input_size=embedding_dim,           # 입력 feature의 수
            hidden_size=h_dim,                  # 은닉 계층의 feature 수
            num_layers=num_layers,              # LSTM 모듈 개수
            dropout=dropout
        )
    ...
    def forward(self, obs_traj):
        ...
        # LSTM 출력: output, (hidden_state, cell_state)
        # output: 각 LSTM 모듈의 hidden state. output[-1] == hidden_state
        # hidden_state, cell_state: 마지막 LSTM 모듈의 결과들
        final_h = state[0]
        return final_h
...
class TrajectoryGenerator(nn.Module):
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, user_noise=None):
        ...
        """
        # obs_traj: (8, N, 2) 크기의 데이터
        # seq_start_end: N개의 데이터 중 i~j까지가 같은 Seq임을 알려주기 위한 리스트
